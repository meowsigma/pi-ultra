...
- [ ] **Step 4: Gate the three executor boundaries** (`executePublic`, `executeDelegated`, `executeScheduled`) before run IDs, workflow storage, missions, schedules, or child processes are created. Bind RPC permits to the normalized params through a module-private WeakMap or explicit non-schema executor argument. The inner recursive `execute` used by an already-admitted workflow is not re-admitted.
- [ ] **Step 5: Prove supported-path closure**. Tool, slash, RPC, and structured bridges already call gated wrappers. Scheduled launches call `executeScheduled`. Authorized Ultra agents run with an inherited capability ceiling that denies ambient extensions and omits the `subagent` tool, so no nested pi-subagents ingress exists inside those children.
- [ ] **Step 6: Set fork version `0.55.1-ultra.0`**, export `./launch-authority`, document the compatibility API, run GREEN/regressions, and commit `feat: enforce launch authority at execution ingress`.
...
- [ ] **Step 3: Implement load results** where `loaded`/`missing` include a SHA-256 revision and `invalid` has no executable settings. Missing defaults remain valid; invalid never exposes enabled defaults to runtime.
...
- Expected output / observable behavior: no input interception/planner request occurs, direct spawn-shaped `subagent` calls block while enabled, safe management remains available, and exactly one terminal follow-up is delivered.
...
- [ ] **Step 2: Write RED operation tests** for append/restore, repair ancestry, completion races, status reconciliation, terminal allowlist, dedupe, actual binding comparison, shutdown abort, and bounded unknown-event buffering.
- [ ] **Step 3: Register authority and capability ceiling by state**. Valid enabled config gets deny-by-default launch authority plus strict agent/tool/extension ceiling. Invalid or incompatible config gets empty `allowedAgents`. Valid off disposes both. Admission revision validation reloads the global config.
...
- [ ] **Step 5: Implement lifecycle persistence** with bounded `pi.appendEntry('ultra.operation.v1', snapshot)`, latest-per-operation restoration from the active branch, race buffer, one status/replay query, terminal dedupe, expected-vs-actual evidence, and `pi.sendMessage(..., {triggerTurn:true, deliverAs:'followUp'})`.
...
- [ ] **Step 5: Add blocked-config backup/reset action**, update truthful help/copy, run GREEN/regressions, and commit `feat: finish ultra model and lane controls`.
...
- [ ] **Step 2: Pin compatibility** to exact `pi-subagents@0.55.1-ultra.0`, add `typebox`, add `smoke:packed`, and ensure package files include strict agents but no planner prompts.
...
- [ ] **Step 6: Push fork branch/tag and pi-ultra branch**, install the exact fork and refreshed pi-ultra Git package under `~/.pi/agent`, run `/reload`, and verify live `/ultra` status/menu plus a blocked under-minimum launch and authorized four-lane wave.
- [ ] **Step 7: Merge pi-ultra through a normal merge commit, push main, and record final SHAs/test evidence**. Open an upstream pi-subagents PR for the generic launch-authority API; the production install remains pinned to the reviewed fork until upstream publishes compatibility.
